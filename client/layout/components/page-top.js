...
import {SearchBar} from 'client/layout/components/search-bar';

// Lib
import {MessagesAlert, MessagesAlertContainer, NotificationsAlert, NotificationAlert, eventBus} from 'react-blur-admin';
...
export class PageTop extends React.Component {
  ...
  renderLogo() {
    return (
      <Link to={{pathname: '/'}} className="al-logo clearfix">{process.env.APP_NAME}</Link>
    );
  }
  ...
  renderLoggedOutUser() {
    let login = <li><a href="#">Login Disabled</a></li>;
    if (process.env.AUTH0_PUB_KEY && process.env.AUTH0_DOMAIN) {
      login = <li><Link to="/login"><i className="fa fa-user"></i>Login</Link></li>;
    }
    return (
  ...
  renderUserSection() {
    if (_.isEmpty(this.props.user)) {
      return this.renderLoggedOutUser();
    }
