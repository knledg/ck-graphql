...
import {AppRouter} from 'client/init/router';
import {setGQLConfig} from 'client/components/graphql-container';
...
function init() {
  const userToken = localStorage && localStorage.getItem ? localStorage.getItem('userToken') : '';
  setGQLConfig({
    graphqlUrl: `${process.env.API_BASE}graphql`,
    networkHeaders: {Authorization: `Bearer ${userToken}`},
  });
